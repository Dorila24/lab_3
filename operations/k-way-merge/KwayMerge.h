                             ...
                             std::size_t block_size = 8)
{
    ...
    for (std::size_t i = 0; i < array.size(); i += block_size) {
        const std::size_t j = std::min(i + block_size, array.size());
        std::vector<T> chunk(array.begin() + static_cast<std::ptrdiff_t>(i),
                             array.begin() + static_cast<std::ptrdiff_t>(j));
    ...
    while (runs.size() > 1) {
        ...
        for (std::size_t i = 0; i < runs.size(); i += k) {
            const std::size_t end = std::min(i + k, runs.size());
